...
from transformers.models.gptj.modeling_gptj import GPTJConfig, GPTJModel, GPTJPreTrainedModel
from transformers.utils import ModelOutput

class GPTJRewardModelConfig(GPTJConfig): 
    model_type = "gpt_j_reward_model"
...
@dataclass
class GPTJRewardModelOutput(ModelOutput):
    """
    Reward model output.
    Args:
...
class GPTJRewardModel(GPTJPreTrainedModel):  # pylint: disable=abstract-method
    config_class = GPTJRewardModelConfig

    def __init__(self, config):
        if isinstance(config, GPTJConfig):
            # When a normal GPTJ was loaded it will be converted into a reward model.
            # The direct `type(config) == GPTJConfig` comparison is used (instead of
            # `isinstance()`) since the configuration class of the reward model is also
            # derived form `GPTJConfig`.
            config = GPTJRewardModelConfig.from_dict(config.to_dict())
    ...
    def forward(
        ...
        **kwargs,
        ...
        return GPTJRewardModelOutput(logits=logits)
...
class GPTNeoXRewardModel(GPTNeoXPreTrainedModel):  # pylint: disable=abstract-method
...
AutoConfig.register("gpt_j_reward_model", GPTJRewardModelConfig)
AutoModelForSequenceClassification.register(GPTJRewardModelConfig, GPTJRewardModel)
